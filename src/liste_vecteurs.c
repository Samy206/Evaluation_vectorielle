...
te_variable vars[] = { {"a", &T[0]}, {"b", &T[1]}, {"c", &T[2]}, {"d", &T[3]}, {"f", &T[4]},
						   {"g", &T[5]}, {"h", &T[6]}, {"i", &T[7]}, {"j", &T[8]}, {"k", &T[9]},
						   {"l", &T[10]}, {"m", &T[11]}, {"n", &T[12]}, {"o", &T[13]}, {"p", &T[14]},
						   {"q", &T[15]}, {"r", &T[16]}, {"s", &T[17]}, {"t", &T[18]}, {"u", &T[19]},
						   {"v", &T[20]}, {"w", &T[21]}, {"x", &T[22]}, {"y", &T[23]}, {"z", &T[24]},
					 };
...
int* retourne_nombre_inconnues(char **chaine)
{
	...
	for(int i=0; i<nb_fonctions; ++i)
	{
		...
		else
		{
			...
			{
				if ( (j == 0) && (!est_une_lettre(chaine[i][j+1], 0, 25)) )
				{
					if (est_une_lettre(chaine[i][j], 0, 25)) t[i]++;
				}
				else if ((j < index) && (!est_une_lettre(chaine[i][j-1], 0, 25) && !est_une_lettre(chaine[i][j+1], 0, 25)))
				{
					if (est_une_lettre(chaine[i][j], 0, 25)) t[i]++;
				}
			}
			if (est_une_lettre(chaine[i][index], 0, 25))
				if ( !est_une_lettre(chaine[i][index-1],0,25) ) t[i]++;
		}
	}
	return t;
}
	...
	for(int i=0; i<nb_fonctions; ++i)
	{
		...
		else
		{
			for(int j=0; j<strlen(chaine[i]); ++j)
			{
				if ( (j == 0) && (!est_une_lettre(chaine[i][j+1], 0, 25)) )
				{
					if (est_une_lettre(chaine[i][j], 0, 25))
					{
						msg[k] = chaine[i][j];
						k++;
					}
			...
			if (est_une_lettre(chaine[i][index], 0, 25))
				if ( !est_une_lettre(chaine[i][index-1],0,25) )
				{
		...
		if(expr_2)
		{
			...
			for(int j=0; j<T1[i]; ++j)
			{
				for(int k=0; k<v_precedent.taille; ++k)
				{
					...
					{
						T[recherche_dichotomique(T2[i][j], 0, 25)] = v_precedent.tableau[k];
					}
				}
			}
